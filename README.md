...
<details>
  ...
  <ol>
    ...
    <li><a href=" #hardware-design-and-implementation">Hardware Design and Implementation</a></li>
    <li><a href=" #software-implementation">Software Implementation</a></li>
    <li><a href=" #testing-and-performance-evaluation">Testing and Performance Evaluation</a></li>
...
<p align="justify">Kelembapan tanah merupakan salah satu faktor penting dalam keberhasilan pertumbuhan tanaman. Dalam praktiknya, pemantauan dan pengaturan kelembapan tanah masih banyak dilakukan secara manual oleh petani atau pemilik lahan, yang tentu memerlukan waktu, tenaga, dan keterlibatan secara langsung. Ketika terjadi kelalaian dalam penyiraman atau tidak adanya pemantauan secara berkala, kondisi tanah dapat menjadi terlalu kering atau terlalu basah. Hal ini dapat berdampak buruk terhadap pertumbuhan dan kesehatan tanaman, serta menurunkan hasil panen secara keseluruhan.
</p>
<p align="justify">Di sisi lain, kemajuan teknologi sistem embedded dan sensor digital membuka peluang untuk merancang sistem monitoring kelembapan tanah secara otomatis yang dapat bekerja secara real-time dan mandiri. Namun, implementasi sistem seperti ini masih memiliki tantangan, seperti keterbatasan dalam konsumsi daya, respons terhadap perubahan kondisi lingkungan, serta integrasi antara komponen sensor, aktuator, dan mikrokontroler.
</p>

### 1.2 Proposed Solution
<p align="justify">Dalam proyek ini, sistem dirancang untuk mendeteksi tingkat kelembapan tanah secara otomatis dan real-time menggunakan sensor kelembapan digital (DHT11) yang terhubung dengan mikrokontroler slave. Data kelembapan dikirimkan ke mikrokontroler master melalui komunikasi SPI (Serial Peripheral Interface). Berdasarkan nilai kelembapan yang diterima, sistem akan menentukan apakah pompa air perlu diaktifkan.
</p>

Komponen utama dari solusi kami meliputi:
- **Sensor DHT11**: Digunakan untuk mengukur kelembapan secara real-time. Data kelembapan yang diperoleh akan menjadi dasar pengambilan keputusan sistem.
...
- **LED Indikator**: LED digunakan sebagai indikator status kelembapan tanah (merah untuk tanah kering, hijau untuk tanah cukup lembap).
...
- **DHT11_reading**: Bertugas membaca data bit-bit kelembapan yang dikirimkan sensor secara berurutan, kemudian mengonversi bit-bit tersebut menjadi nilai kelembapan dalam format byte.
- **SPI_send_data**: Digunakan untuk mengirimkan data kelembapan dari mikrokontroler pengukur ke mikrokontroler pengendali melalui protokol SPI.
- **LED_control**: Bertanggung jawab mengatur status LED indikator berdasarkan nilai kelembapan yang diterima.
...
- **pump_control**: Mengatur kondisi pompa air berdasarkan nilai kelembapan dan timer.
...
- Pembacaan nilai kelembapan tanah oleh sensor DHT11
- Transmisi data yang benar antara mikrokontroler slave dan master
- Aktivasi pompa air yang sesuai berdasarkan pembacaan kelembapan
- Fungsionalitas tombol kontrol manual untuk durasi penyiraman
- Indikasi visual melalui LED yang merepresentasikan status kelembapan tanah
...
- Sensor DHT11 mampu membaca nilai kelembapan tanah dengan akurat dan mengirimkan data secara tepat ke mikrokontroler.
- Respons sistem terhadap perubahan kelembapan juga berjalan dengan baik, dimana pompa air aktif secara otomatis saat kelembapan turun di bawah batas yang telah ditentukan.
...
- LED indikator berhasil menampilkan status kelembapan secara visual dengan benar, memudahkan pemantauan kondisi tanah secara langsung.
...
- Mengintegrasikan sistem dengan aplikasi mobile atau IoT agar monitoring kelembapan tanah dan pengendalian pompa bisa dilakukan secara jarak jauh.
- Meningkatkan penggunaan sensor dengan akurasi lebih tinggi untuk pembacaan kelembapan yang lebih presisi.
